...
In order to run this Jenkins with docker container in EC2 instance, run `docker run -p 50000:50000 -p 8080:8080 -v /var/jenkins_home:/var/jenkins_home -v /run/docker.sock:/run/docker.sock --name jenkins -d jenkins-with-docker-image`.
...
* Github Integration Plugin (jenkins to start build automatically when a new commit is done on the Github repo)
...
* In Build, select Process Job DSL's and add the path `App/nodejs_application.groovy` for DSL script. 
This creates a seed project, which will create a pipeline `Nodejs Application` on successful completion of build. On adding a new commit on github will automatically start a new build.
...
Here is the URL for the application which is running on an EC2 Instance - http://34.213.6.106:3000/

(b) Jenkins Pipeline using Jenkinsfile - <b> project-pipeline</b></br>
This approach is better than Job DSl. It handles different stages like build/test/deployment of a single project. Steps are written in the `Pipeline/Jenkinsfile`.
Steps -
...
(iv). docker build/ publish </br>
      build the docker image on the basis of proviede Dockerfile and push the image to dockerhub. Tags of images are commit id.  </br>
...
First the basic approach is used to deploy kubernetes objects using Kubectl(command line tool)
...
Then I created a blue-green deployment. They are - blue-deployment.yaml, green-deployment.yaml and loadbalancer-service.yaml. Both deployments have different app version and are using different images. Loadbalancer will forward the traffic to the deployment whose version is mentioned in loadbalancer-service.yaml file.

<img width="1303" alt="Screenshot 2020-07-06 at 2 52 41 PM" src="https://user-images.githubusercontent.com/25201552/86577921-9bea9e00-bf98-11ea-9891-50a9b9ec109c.png">
...
### (5) Helm Deployment
Rather than running all the menifest.yaml files indivisually, one better approach is to use Helm Chart for Kubernetes Objects deployment. I have created a helm chart `Node-Chart'`. Blue-Green deployment is done with the help of that helm chart. I have separated all the values from the yaml files and created a separate file - values.yaml. Now in order to make any changes regarding image of any of the deployment, namespace or version one only has to edit the values.yaml file. Those values will be combined with the template while running `helm install` command. 
The command used is `helm install nodejs-app Node-Chart`.
Namespace used is -   `helm-deployment`.
